  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-2xl font-bold text-slate-900 mb-1">Get Instant Price Estimate</h2>
        <p className="text-xs text-slate-500">Regression valuation based on Kaggle flat market pricing model (XGBoost R²=0.7467). Disclaimer: This predicts property transaction values, not direct raw material construction budgets.</p>
      </motion.div>
              ...
              <motion.div key="result" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                <div className="glass-card p-6 text-center">
                  ...
                  <div className="text-sm text-slate-400">
                    {formatINR(Math.round(result.predicted_cost / form.area))}/sq ft • Calculated from project inputs
                  </div>
                  {result.fallback && (
