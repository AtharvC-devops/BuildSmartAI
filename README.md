...
The backend uses a local MySQL database named `buildsmartai`. SQLite is retained only as an explicit one-time import source for existing development data; it is not a runtime backend dependency.

### Prerequisites

- Node.js 18+
- MySQL 8+ and the MySQL service running
- npm

### MySQL setup

1. Start the MySQL service.
2. Create the database or let the migration create it:
   ```sql
   CREATE DATABASE buildsmartai CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
   ```
3. Copy `backend/.env.example` to `backend/.env` and set `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, and `DB_NAME=buildsmartai`.
...
5. Seed development data:
   ...
   npm run db:seed
   ```
6. If an existing `backend/buildsmart.db` must be preserved, configure MySQL first and run:
   ```bash
   npm run db:import-sqlite
   ```
   The importer reports migrated and failed records and hashes legacy plaintext development passwords during import.
7. Start the backend:
   ```bash
   npm start
   ```
...
- **MySQL not running:** start the MySQL80 service and retry `npm run db:migrate`.
- **Access denied:** verify `DB_USER` and `DB_PASSWORD` in `backend/.env`.
- **Unknown database:** use `DB_NAME=buildsmartai` and rerun `npm run db:migrate`.
- **Port already in use:** set a different `PORT` for the backend or stop the existing process.
- **Migration failure:** inspect the first SQL error, correct the MySQL account/database configuration, then rerun the idempotent migration.
...
- **JWT & scrypt Authentication**: Passwords are stored as salted Node.js `scrypt` hashes. Stateless session tokens are signed with JSON Web Tokens.
...
| **Database** | Local MySQL 8 with pooled `mysql2` connections and versioned SQL migrations |
...
│   ├── src/
│   │   ├── data/
│   │   │   └── sampleData.js       # Legacy UI/demo fallback data
│   │   ├── middleware/
