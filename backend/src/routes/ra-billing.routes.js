 ...
 * Returns all site measurements for a project, enriched with BOQ item description
 */
router.get("/projects/:id/measurements", asyncWrap(async (req, res) => {
  const projectId = parseInt(req.params.id);
  const { status, boqItemId } = req.query;

  let sql = `
    SELECT sm.*,
           bi.description AS boq_description,
           bi.category    AS boq_category,
           bi.quantity    AS contract_qty,
           bi.rate        AS contract_rate
  ...
  if (boqItemId) { sql += " AND sm.boq_item_id = ?"; params.push(parseInt(boqItemId)); }
 ...
 * Record a new site measurement (starts in DRAFT status)
 */
router.post("/projects/:id/measurements", asyncWrap(async (req, res) => {
  const projectId = parseInt(req.params.id);
  const { boqItemId, measurementDate, description, quantity, unit, recordedBy } = req.body;

  if (!boqItemId || !measurementDate || !quantity) {
    throw { status: 400, code: "MISSING_FIELDS", message: "boqItemId, measurementDate, and quantity are required" };
  }

  // Verify BOQ item belongs to project
  const boqItem = await get(
    "SELECT * FROM boq_items WHERE id = ? AND project_id = ?",
    [parseInt(boqItemId), projectId]
  );
  if (!boqItem) throw { status: 404, code: "BOQ_ITEM_NOT_FOUND", message: "BOQ item not found in this project" };

  const result = await run(
    `INSERT INTO site_measurements
       (project_id, boq_item_id, measurement_date, description, quantity, unit, recorded_by, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'draft')`,
    [
      projectId,
      parseInt(boqItemId),
  ...
  const created = await get("SELECT sm.*, bi.description AS boq_description FROM site_measurements sm JOIN boq_items bi ON bi.id = sm.boq_item_id WHERE sm.id = ?", [result.lastInsertRowid]);
  return successResponse(res, created, 201);
}));

/**
 * PATCH /api/projects/:id/measurements/:measId/status
 * Transition measurement status (verify or reject)
 * Only VERIFIED measurements can be included in an RA Bill
 */
router.patch("/projects/:id/measurements/:measId/status", asyncWrap(async (req, res) => {
  const projectId = parseInt(req.params.id);
  const measId = parseInt(req.params.measId);
  const { status, rejectionReason } = req.body;

  const norm = normalizeStatus(status);
  if (!["verified", "rejected"].includes(norm)) {
    throw { status: 400, code: "INVALID_STATUS", message: "Measurement status must be 'verified' or 'rejected'" };
  }
  ...
  if (!meas) throw { status: 404, code: "MEASUREMENT_NOT_FOUND", message: "Measurement not found" };
  if (meas.is_billed) throw { status: 409, code: "ALREADY_BILLED", message: "Cannot change status of an already-billed measurement" };

  await run(
    "UPDATE site_measurements SET status = ?, rejection_reason = ? WHERE id = ?",
    [norm, rejectionReason || null, measId]
  );

  const updated = await get("SELECT * FROM site_measurements WHERE id = ?", [measId]);
