...
const { calculateAgentScore, calculateSupplierScore } = require("../utils/scoring");
...
const costSchema = {
  ...
  location_tier: { type: "number", required: true, integer: true, min: 1, max: 3 },
  floors: { type: "number", required: true, integer: true, positive: true }
};

const timeSchema = {
  area: { type: "number", required: true, positive: true },
  workers: { type: "number", required: true, integer: true, positive: true },
  complexity: { type: "number", required: true, integer: true, min: 1, max: 5 }
};
...
const priceSchema = {
  ...
  total_sqft: { type: "number", required: true, positive: true },
  bhk: { type: "number", required: true, integer: true, positive: true },
  bath: { type: "number", required: true, integer: true, positive: true }
};
...
// Helper wrapper to handle express async exceptions
const asyncHelper = (fn) => (req, res, next) => {
  Promise.resolve(fn(req, res, next)).catch(next);
};

// ── POST /api/predict-cost ──────────────────────────────────────────────
router.post("/predict-cost", validateSchema(costSchema), asyncHelper(async (req, res) => {
  try {
    ...
    return successResponse(res, {
      ...
      calculationMethod: "area x quality x location x floors",
      fallback: true,
    });
  }
}));
...
router.post("/predict-time", validateSchema(timeSchema), asyncHelper(async (req, res) => {
  try {
    ...
    return successResponse(res, {
      ...
      calculationMethod: "area and complexity adjusted by workforce capacity",
      fallback: true,
    });
  }
}));
...
router.post("/assign-agent", validateSchema(agentSchema), asyncHelper(async (req, res) => {
  try {
    ...
    const rankings = agents
      .map((a) => ({
        ...a,
        score: calculateAgentScore(a, required_skill)
      }))
      .sort((a, b) => b.score - a.score);

    return successResponse(res, { rankings, best_agent: rankings[0] || {}, fallback: true });
  }
}));

// ── POST /api/predict-materials ─────────────────────────────────────────
router.post("/predict-materials", validateSchema(materialsSchema), asyncHelper(async (req, res) => {
  try {
    ...
    return successResponse(res, {
      ...
      calculationMethod: "quantity factors derived from area, floors, quality, and construction type",
      fallback: true,
    });
  }
}));
...
router.post("/predict-risk", validateSchema(riskSchema), asyncHelper(async (req, res) => {
  try {
    ...
    return successResponse(res, {
      ...
      calculationMethod: "rule-based safety, weather, and compliance checklist",
      fallback: true,
    });
  }
}));
...
// ── GET /api/suppliers ──────────────────────────────────────────────────
router.get("/suppliers", asyncHelper(async (req, res) => {
...
// ── POST /api/qa-chat ────────────────────────────────────────────────────
router.post("/qa-chat", validateSchema(chatSchema), asyncHelper(async (req, res) => {
  try {
    ...
    return successResponse(res, { answer, fallback: true });
  }
}));

// ── POST /api/predict-price (Real ML Model) ─────────────────────────────
router.post("/predict-price", validateSchema(priceSchema), asyncHelper(async (req, res) => {
  try {
    const { data } = await axios.post(`${AI_SERVICE_URL}/predict-price`, req.body);
    return successResponse(res, data);
  } catch (err) {
    console.error(`[ERROR] AI predict-price failed:`, err.message);
    throw { status: 500, code: "AI_SERVICE_ERROR", message: "AI service unavailable. Ensure model is trained." };
  }
}));

// ── GET /api/locations (Model Location List) ────────────────────────────
router.get("/locations", asyncHelper(async (req, res) => {
  try {
    const { data } = await axios.get(`${AI_SERVICE_URL}/locations`);
    return successResponse(res, data);
  } catch (err) {
    console.warn("[WARN] AI service unavailable for locations");
    return successResponse(res, { locations: [], count: 0, fallback: true });
  }
}));
